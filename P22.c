...
double lineMagnitude(Point lineStart, Point lineEnd, Point point); //Return the distance between two points
void generateHull(Point* points, Point* hull, int size, Point lineStart, Point lineEnd, int side); //Finds and stores the extreme points
int sideCheck(Point lineStart, Point lineEnd, Point point);
void saveToArr(Point* hull, Point toSave);
void minMax(Point* points, int* min, int* max);
...
int main(int argc, char* argv[]) {
    ...
    Point* hull = (Point*)calloc(MAX_EXTREME, sizeof(Point)); //Initialize hull array
    ...
    minMax(points, &xMin, &xMax);

    begin = clock(); //Begin clock
    generateHull(points, hull, 30000, points[xMin], points[xMax], 1);
    generateHull(points, hull, 30000, points[xMin], points[xMax], -1);
    end = clock(); //End clock
...
double lineMagnitude(Point lineStart, Point lineEnd, Point point) {
    return abs((point.y - lineStart.y) * (lineEnd.x - lineStart.x) - (lineEnd.y - lineStart.y) * (point.x - lineStart.x));
}

void generateHull(Point* points, Point* hull, int size, Point lineStart, Point lineEnd, int side) {
    int ptPtr = -1; //Index of third point (pt in question)
    int magnitude = 0; //Store the Largest line segment

    for (int i = 0; i < size; i++) {
        double lineDist = lineMagnitude(lineStart, lineEnd, points[i]); //Store
        if (sideCheck(lineStart, lineEnd, points[i]) == side && lineDist > magnitude) {
            ptPtr = i;
            magnitude = lineDist;
        }
    }
    ...
    if (ptPtr == -1) {
        saveToArr(hull, lineStart);
        saveToArr(hull, lineEnd);
        return;
    }

    generateHull(points, hull, size, points[ptPtr], lineStart, -sideCheck(points[ptPtr], lineStart, lineEnd));
    generateHull(points, hull, size, points[ptPtr], lineEnd, -sideCheck(points[ptPtr], lineEnd, lineStart));
}

int sideCheck(Point lineStart, Point lineEnd, Point point) {
    if ((point.y - lineStart.y) * (lineEnd.x - lineStart.x) - (lineEnd.y - lineStart.y) * (point.x - lineStart.x) > 0) {
        return 1;
    }
    else if ((point.y - lineStart.y) * (lineEnd.x - lineStart.x) - (lineEnd.y - lineStart.y) * (point.x - lineStart.x) == 0) {
        return 0;
    }
    else {
        return -1;
...
void saveToArr(Point* hull, Point toSave) {
    int duplicate = 0;
    for (int i = 0; i < 300; i++) {
        if (hull[i].x == toSave.x && hull[i].y == toSave.y) {
            duplicate = 1;
            break;
        }
    }
...
void findShortestPath(Point* hull, int start, int end, int size) {
    ...
    if (cw < countercw) {
        ...
        if (lCount == 0) {
            ...
            printf("%d. %.1lf, %.1lf\n", end + 1, hull[end].x, hull[end].y); //Printe end
        }
        else {
            printf("%d. %.1lf, %.1lf\n", start + 1, hull[start].x, hull[start].y); //Print start
